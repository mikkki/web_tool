...
app.factory('Session', ['$resource', '$cookieStore', function($resource, $cookieStore) {
        var cid = 'bacster-session'; // cookie id
	var data = $cookieStore.get(cid) || {};

	return $resource('/crud/sessioninfo/', 
			 {'pioneer_id': data().pioneerId, 'notes': data().notes}, 
...
app.controller('searchController', function($scope, $state, $http, 
  ...
  $scope.models = Session.get(); // object
...
//  var session_object = Session.get({pioneer_id: session.data().pioneerId, notes: session.data().notes});
  ...
  console.log("Session model: " + Session.get({pioneer_id: session.data().pioneerId, notes: session.data().notes}));  

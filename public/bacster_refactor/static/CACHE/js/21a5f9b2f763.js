...
app.factory('Session', ['$resource', function($resource) {
	return $resource('bacster/crud/sessioninfo', {"pk": "@pk"}, {'query':  {method:'GET', isArray:false}});  
}]);
...
app.controller('searchController', function($scope, $state, $http, $resource, $cookieStore,
  ...
  var test = new Session_pid('bacster/crud/sessioninfo_pid/' + session.data().user.pioneerId).query(function(data){
        console.log("test get stuff: " + JSON.stringify(data));
  });
  var sess = $resource('bacster/crud/sessioninfo/', {"pk": "@pk"});

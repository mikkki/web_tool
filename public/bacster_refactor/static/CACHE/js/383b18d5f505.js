...
app.factory('Session', ['$resource', function($resource) {
	return $resource('/crud/sessioninfo', {"pk": "@pk"}, {'query':  {method:'GET', isArray:fasle}});  
}]);

app.factory('mySession', ['$resource', function($resource) {
    return $resource('crud/sessioninfo/:pk', { pk:'@pk' }, {
      'get': { method: 'GET'},
      'save':   {method:'POST'},
...
app.controller('searchController', function($scope, $state, $http, $resource,
  ...
  var entry = Session.query({pk: 33});
  console.log("entry: " + JSON.stringify(entry));

  // Getting the current Session object
  var sess = $resource('crud/sessioninfo/', {pk: 5});

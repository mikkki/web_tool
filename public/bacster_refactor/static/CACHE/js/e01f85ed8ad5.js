...
//  var session_object = Session.query({pioneer_id: session.data().pioneerId, notes: session.data().notes});
  var sess = $resource('crud/sessioninfo/:pioneer_id', {pioneer_id:'@pioneer_id'});
  sess.query({pioneer_id: '343'}, function(s, getResponseHeaders){
      console.log("my database session: " + s[0].pioneer_id);  
  });

  var el5 = sess.get({ pioneer_id: '343' });

  console.log("selected record: " + el5.pioneer_id);
